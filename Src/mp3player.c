...
uint16_t g_pMp3OutBuffer[MAX_NCHAN * MAX_NGRAN *
                         MAX_NSAMP]; // output samples from decoder
uint16_t *g_pMp3OutBufferPtr = NULL;
uint16_t
    g_pMp3DmaBuffer[MP3_DMA_BUFFER_SIZE]; // samples to be send to audio codec
 ...
 */
uint32_t Mp3ReadId3V2Text(FIL *pInFile, uint32_t unDataLen, char *pszBuffer,
 ...
 * http://www.mikrocontroller.net/topic/252319
 */
uint32_t Mp3ReadId3V2Tag(FIL *pInFile, char *pszArtist, uint32_t unArtistSize,
                         char *pszTitle, uint32_t unTitleSize) {
  pszArtist[0] = 0;
  pszTitle[0] = 0;
    ...
    if ((unRead == 10) && (id3hd[0] == 'I') && (id3hd[1] == 'D') &&
      ...
      while (nFramesToRead > 0) {
        ...
        if (strcmp(szFrameId, "TPE1") == 0) {
          // artist
          if (Mp3ReadId3V2Text(pInFile, unFrameSize, pszArtist, unArtistSize) !=
              0) {
            break;
          }
          nFramesToRead--;
          ...
          // title
          if (Mp3ReadId3V2Text(pInFile, unFrameSize, pszTitle, unTitleSize) !=
              0) {
            break;
          }
          nFramesToRead--;
...
void PlayMp3File() {
  ...
  char szTitle[120];
  id3tagsize = 0;
  Mp3ReadId3V2Tag(&MyFile, szArtist, sizeof(szArtist), szTitle,
                  sizeof(szTitle));
  bOutOfData = 0;
  ...
  do {
    ...
    if (nOffset < 0) {
      ...
      // decode one MP3 frame - if offset < 0 then bytesLeft was less than a
      // full frame
      nDecodeRes = MP3Decode(hMP3Decoder, &pInData, (int *)&unInDataLeft,
                             (short *)g_pMp3OutBuffer, 0);
      switch (nDecodeRes) {
      case ERR_MP3_NONE: {
        ...
        while (unOutBufferAvail > 0) {
          ...
          if (unDmaBufMode == 0) {
            ...
            unDmaBufferSpace =
                g_pMp3DmaBuffer + MP3_DMA_BUFFER_SIZE - g_pMp3DmaBufferPtr;
          } else if (unDmaBufMode == 1) {
            // fill the first half of the buffer
            // dma buf ptr was reset to beginning of the buffer
            unDmaBufferSpace = g_pMp3DmaBuffer + (MP3_DMA_BUFFER_SIZE / 2) -
                               g_pMp3DmaBufferPtr;
          } else {
          ...
          uint32_t unCopy = unDmaBufferSpace > unOutBufferAvail
                                ? unOutBufferAvail
                                : unDmaBufferSpace;
          if (unCopy > 0) {
            memcpy(g_pMp3DmaBufferPtr, g_pMp3OutBufferPtr,
                   unCopy * sizeof(uint16_t));
          ...
          if (unDmaBufferSpace == 0) {
            ...
            if (unDmaBufMode == 0) {
              ...
              if (mp3FrameInfo.samprate > 0)
                trak.bitrate = mp3FrameInfo.samprate;
              else
                trak.bitrate = AUDIO_FREQUENCY_44K;
              BSP_AUDIO_OUT_ClockConfig(
                  &hsai_BlockA2,
                  (uint32_t)(trak.bitrate / 2) * (1 + trak.percent), NULL);
              BSP_AUDIO_OUT_Play(g_pMp3DmaBuffer,
                                 MP3_DMA_BUFFER_SIZE * sizeof(uint16_t));
