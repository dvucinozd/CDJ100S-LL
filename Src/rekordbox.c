...
static uint8_t FindToken(char *token) {
  ...
  while (f_tell(&MyFile) < f_size(&MyFile)) {
    while (f_tell(&MyFile) < f_size(&MyFile)) {
      if (f_read(&MyFile, &tag[0], 1, (void *)&bytesread) != FR_OK)
        return 1;
      if (tag[0] == 'P')
        break;
    }
    if (f_read(&MyFile, &tag[1], 3, (void *)&bytesread) != FR_OK)
      return 1;
    tag[4] = '\0';
...
static int32_t GetLongNumber() {
  ...
  for (int i = 0; i < 5; i++)
    tag[i] = 0;
  if (f_read(&MyFile, &tag[0], 4, (void *)&bytesread) != FR_OK)
    return 0;
  for (i = 0; i < 4; i++) {
    out_data |= tag[i];
    out_data <<= 8;
...
uint8_t DecodeRekordboxFiles(TCHAR *path) {
  ...
  for (i = 0; i < 1000000; i++)
    *(__IO uint8_t *)(WAVE_BUFFER + i) = 0;
  for (i = 0; i < 255; i++)
    rekordbox.filename[i] = 0;
  char filename[255] = {0};
  TCHAR my_path[255] = {0};
  wcscpy(my_path, path);
  ...
  if (res == FR_OK) {
    if (FindToken(path_token) != 0)
      return 1;
    if (f_read(&MyFile, BufferCtl.buff, 8, (void *)&bytesread) != FR_OK)
      return 1; // dummy read 8 bytes
    data_size = GetLongNumber();
    if (f_read(&MyFile, filename, data_size, (void *)&bytesread) != FR_OK)
      return 1;
    i = 0;
    k = 0;
    while (i < data_size) {
    ...
    if (FindToken(qtz_token) != 0)
      return 1;
    if (f_read(&MyFile, BufferCtl.buff, 16, (void *)&bytesread) != FR_OK)
      return 1; // dummy read 16 bytes
    data_size = GetLongNumber();
    rekordbox.timezones = data_size;
    for (i = 0; i < rekordbox.timezones; i++) {
      for (k = 0; k < 2; k++) {
        if (f_read(&MyFile, &tag[k], 1, (void *)&bytesread) != FR_OK)
          return 1;
      }
    ...
    if (rekordbox.phase[0] == 1)
      rekordbox.beat_grid_offset = 4;
    else
      rekordbox.beat_grid_offset = rekordbox.phase[0] - 1;
    if (FindToken(wave_token) != 0)
      return 1;
    if (f_read(&MyFile, BufferCtl.buff, 8, (void *)&bytesread) != FR_OK)
      return 1; // dummy read 8 bytes
    rekordbox.lowp_spectrum_size = GetLongNumber();
    if (f_read(&MyFile, BufferCtl.buff, 4, (void *)&bytesread) != FR_OK)
      return 1; // dummy read 4 bytes
    if (f_read(&MyFile, &lowp_wavebuffer[0], rekordbox.lowp_spectrum_size,
               (void *)&bytesread) != FR_OK)
      return 1;

    if (FindToken(cob_token) != 0)
      return 1;
    if (f_read(&MyFile, BufferCtl.buff, 12, (void *)&bytesread) != FR_OK)
      return 1; // dummy read 12 bytes
    rekordbox.cues = GetLongNumber() + 1;
    for (i = 1; i < rekordbox.cues; i++) {
      if (FindToken(cue_token) != 0)
        return 1;
      if (f_read(&MyFile, BufferCtl.buff, 8, (void *)&bytesread) != FR_OK)
        return 1; // dummy read 8 bytes
      ...
      if (f_read(&MyFile, &rekordbox.cue_singleloop[i], 1,
                 (void *)&bytesread) != FR_OK)
        return 1;
      if (f_read(&MyFile, BufferCtl.buff, 3, (void *)&bytesread) != FR_OK)
        return 1; // dummy read 3 bytes
      rekordbox.cue_start_position[i] =
          (int32_t)((float)GetLongNumber() * 150 / 1000);
      if (rekordbox.cue_singleloop[i] == 2)
        rekordbox.cue_end_position[i] =
            (int32_t)((float)GetLongNumber() * 150 / 1000);
    }
    if (FindToken(cob_token) != 0)
      return 1;
    ...
    for (k = i; k < rekordbox.cues; k++) {
      ...
      if (f_read(&MyFile, &rekordbox.cue_singleloop[k], 1,
                 (void *)&bytesread) != FR_OK)
        return 1;
      if (f_read(&MyFile, BufferCtl.buff, 3, (void *)&bytesread) != FR_OK)
        return 1; // dummy read 3 bytes
      rekordbox.cue_start_position[k] =
          (int32_t)((float)GetLongNumber() * 150 / 1000);
      if (rekordbox.cue_singleloop[k] == 2)
        rekordbox.cue_end_position[k] =
            (int32_t)((float)GetLongNumber() * 150 / 1000);
    }
    f_close(&MyFile);
  }
    ...
    for (i = 0; i < rekordbox.spectrum_size; i++) {
      ...
      *(__IO uint8_t *)(WAVE_BUFFER + i) = tag[0];
      if (rekordbox.autocue == 1) {
        if (((tag[0] & 0x1F) > acue_sensitivity / 10) &&
            ((tag[0] & 0xE0) < 128)) {
          rekordbox.autocue = 2;
  ...
  while (i++ < rekordbox.spectrum_size) {
    if ((i % 9000) == 0) {
      rekordbox.remminticks[l] =
          (uint16_t)(rekordbox.lowp_spectrum_size *
                     (rekordbox.spectrum_size - i) / rekordbox.spectrum_size);
      rekordbox.elminticks[l] = (uint16_t)(rekordbox.lowp_spectrum_size * i /
